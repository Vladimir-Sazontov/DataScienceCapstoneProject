KN <- function(input_text, n_show) {
        ...
        for (i in 1:dim(FTempNew)[1]) {
          ...
          while (j <= len) {
            ...
            if (j == 1) {
              ...
              D <- sort(unique(FTL[[j]]$Freq))[1]/(sort(unique(FTL[[j]]$Freq))[1] + 2*sort(unique(FTL[[j]]$Freq))[1])
              P[i] <- max((FTempNew$Freq[i] - D), 0)/sum(FTempNew$Freq) + D*N/sum(FTempNew$Freq)*P[i]
            }
            j <- j + 1
          }
        }
  ...
  for (i in (nmax-1):1) {
    if (input_text_len >= i) {
      ...
      if (dim(FTemp)[1] != 0) {
        FTemp$P_kn <- P_kn(FTemp, len, FTL)
        FTemp <- FTemp[order(FTemp$P_kn, decreasing = T), ]
      }
      else{
